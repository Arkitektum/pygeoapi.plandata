...
from pygeoapi.util import filter_providers_by_type, filter_dict_by_key_value, render_j2_template, to_json
from pygeoapi.provider.base import ProviderGenericError
from . import APIRequest, API, SYSTEM_LOCALE, F_HTML
...
def get_styles(api: API, request: APIRequest) -> Tuple[Dict, int, str]:
    ...
    #         api.tpl_config, api.config['server']['templates'], 'styles/index.jinja2', content, str(request.locale))
...
def get_collection_styles(api: API, request: APIRequest, collection_id: str) -> Tuple[Dict, int, str]:
    headers = request.get_response_headers(SYSTEM_LOCALE, **api.api_headers)
    ...
    if not collection:
        return api.get_exception(HTTPStatus.NOT_FOUND, headers, request.format, 'NotFound', 'Collection not found')

    provider_def = filter_providers_by_type(collection['providers'], 'style')

    if not provider_def:
        return api.get_exception(HTTPStatus.NOT_FOUND, headers, request.format, 'NotFound', 'Style not found')
...
def get_style(api: API, request: APIRequest, style_id: str) -> Tuple[dict, int, str]:
    headers = request.get_response_headers(SYSTEM_LOCALE, **api.api_headers)
...
def get_style_definition(api: API, request: APIRequest, style_id: str) -> Tuple[dict, int, str]:
    headers = request.get_response_headers(SYSTEM_LOCALE, **api.api_headers)
...
def get_style_metadata(api: API, request: APIRequest, style_id: str) -> Tuple[dict, int, str]:
    headers = request.get_response_headers(SYSTEM_LOCALE, **api.api_headers)
